Berikut adalah **desain lengkap slide presentasi** berbasis **Markdown + Reveal.js** (bisa dibuka di browser, ringan, dan mendukung **live coding demo** via CLI + Golang), **sudah dilengkapi dengan praktik langsung** untuk setiap jenis serangan session hijacking sesuai outline.

> **Aplikasi yang digunakan:**
> - **Reveal.js** (HTML presentation framework) → untuk slide
> - **Go (Golang)** → untuk demo backend (server sederhana)
> - **CLI (Terminal)** → untuk simulasi serangan & pencegahan
> - **Wireshark / tcpdump** → untuk capture packet (opsional)
> - **Burp Suite Community** → untuk demo MITB, Session Fixation, dll (opsional)

---

## CARA MENJALANKAN PRESENTASI INI
1. Simpan semua file di folder `session-hijacking-presentation/`
2. Jalankan `go run server.go` untuk demo server
3. Buka `index.html` di browser → gunakan **Reveal.js**
4. Gunakan terminal terpisah untuk demo serangan

---

# PRESENTASI: Understanding Session Hijacking Attacks and Countermeasures  
*(42 Slide + 12 Praktik Langsung dengan Golang & CLI)*
...
## STRUKTUR FOLDER
```
session-hijacking-presentation/
├── index.html          ← Presentasi Reveal.js
├── server.go           ← Demo server Go
├── go.mod
└── README.md
```
...
## FITUR KHUSUS
- **Live coding** di slide
- **CLI command** langsung bisa dicopy
- **Demo serangan & fix** dalam 1 server
- **Bisa dijalankan offline**
- **Siap untuk workshop 1 hari**

---

**Ingin versi PPTX?** Bisa diekspor dari Reveal.js → PDF → Import ke PowerPoint.

**Ingin tambah video demo?** Rekam dengan `asciinema` dan embed.

---

> **Siap presentasi + praktik langsung!**  
> Cocok untuk **training cybersecurity**, **bootcamp**, atau **workshop internal**.

Butuh versi **PDF**, **PPTX**, atau **Dockerized demo**? Kasih tahu!